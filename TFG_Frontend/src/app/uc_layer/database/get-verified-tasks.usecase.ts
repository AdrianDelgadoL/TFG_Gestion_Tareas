...
export class GetUnverifiedTasksUseCase implements UseCaseTemplate<null, Promise<any>> {
  constructor(private db: DatabaseService,
              private userService: UserService) {
  }
  execute(operator: null): Promise<any> {
    if(this.userService.userPermits.includes("READ")) {
      return this.db.getUnverifiedTasks(this.userService.userId);
    }
    return this.db.getUnverifiedTasks();
  }
}
